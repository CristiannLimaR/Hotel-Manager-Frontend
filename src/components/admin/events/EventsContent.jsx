...
const EventsContent = () => {
  ...
  useEffect(() => {
    // Ensure event data is structured for the new display
  ...
  const handleSubmit = async () => {
    if (!form.nombre_evento || !form.fecha) {
      toast({
    ...
    if (editingEventId) {
      success = await updateEvent(editingEventId, preparedEvent);
    } else {
      success = await createEvent(preparedEvent);
    }

    if (success) {
      toast({
        title: editingEventId ? "Evento actualizado" : "Evento creado",
        description: `El evento fue ${
          editingEventId ? "actualizado" : "agregado"
        } exitosamente.`,
        status: "success",
        duration: 3000,
        isClosable: true,
  ...
  return (
    ...
    >
        ...
        <SimpleGrid columns={{ base: 1, md: 2, lg: 3 }} spacing={6}>
          {filteredEvents.map((event) => (
            <EventCard
              ...
              event={{
                ...event,
                status: event.estado ? "Activo" : "Inactivo" // Ensure status is correctly mapped for card display
              }}
      ...
      <EventFormModal
        ...
        handleRecursosChange={handleRecursosChange}
